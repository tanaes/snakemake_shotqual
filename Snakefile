    ...
    run:
        with tempfile.TemporaryDirectory(dir=TMP_DIR_ROOT) as temp_dir:
            shell("""
                  ...
                  $bowtie2 -p {threads} -x {filter_db} --very-sensitive -U {input.unpaired_1} --un-gz %s/{params.unpaired_1_fn} -S /dev/null 2> {log.other}
                  $bowtie2 -p {threads} -x {filter_db} --very-sensitive -U {input.unpaired_2} --un-gz %s/{params.unpaired_2_fn} -S /dev/null 2> {log.other}
